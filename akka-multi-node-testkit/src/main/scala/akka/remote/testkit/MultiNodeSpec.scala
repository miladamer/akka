...
object MultiNodeSpec {
   ...
   *
   * {{{
   * -Dmultinode.tcp.port=0
   * }}}
   */
  val tcpPort: Int = Integer.getInteger("multinode.tpc.port", 0)

  require(tcpPort >= 0 && tcpPort < 65535, "multinode.tcp.port is out of bounds: " + tcpPort)
  ...
  require(
    udpPort.getOrElse(1) >= 0 && udpPort.getOrElse(1) < 65535,
    "multinode.udp.port is out of bounds: " + udpPort)
